...
public class ClearStatusTest extends TestBase{ 
	
     static Sheet excelSheet;
	
	@Test(dataProvider="info")
	public void channelPageTest(String email,String pass,String channel,String post,String test) throws InterruptedException {
		 getLoginPage();
...
//        Assert.assertFalse(deleteChannel);
		   ...
		   String validateActive =cas.activeStatusValidatePage();
		    ...
		    getSetStatusPage();
		     ...
		     getClearStatusPage();
			    cs.clickStatusPage();
			    String status=cs.statusValidatePage();
			    Assert.assertEquals(status,"Testing");
			     cs.clearStatusPage();
			     cs.clickProfilepage();
			    String clearCheck=cs.clearStatusValidatePage();
			     Assert.assertEquals(clearCheck,"Update your status");
